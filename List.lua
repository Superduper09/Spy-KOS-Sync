...
function Spy:AddKOSData(name)
	...
	SpyPerCharDB.KOSData[name] = timestamp
--	SpyPerCharDB.PlayerData[name].kos = 1
	...
	-- Initialize kosAddedBy and track ourselves
	local playerData = SpyPerCharDB.PlayerData[name]
	if playerData then
		if not playerData.kosAddedBy then
			playerData.kosAddedBy = {}
		end
		playerData.kosAddedBy[Spy.CharacterName] = timestamp
		if not playerData.guildStats then
			playerData.guildStats = {}
		end
	end
	-- Broadcast to guild
	Spy:BroadcastKOSAdd(name, timestamp, nil, nil)
end

-- Broadcast KOS add to guild
function Spy:BroadcastKOSAdd(name, timestamp, reasonKey, reasonText)
	if not Spy.db.profile.ShareKOSReasons then return end
	if not GetGuildInfo("player") then return end

	-- Get player info to include in broadcast
	local playerData = SpyPerCharDB.PlayerData[name]
	local class = playerData and playerData.class or ""
	...
	local escapedName = Spy:EscapeSyncString(name)
	local escapedKey = Spy:EscapeSyncString(reasonKey or "")
	local escapedText = Spy:EscapeSyncString(reasonText or "")
...
function Spy:RemoveKOSData(name)
	if SpyPerCharDB.KOSData[name] then
		...
		Spy:BroadcastKOSRemoval(name)
	end
end

-- Broadcast KOS removal to guild
function Spy:BroadcastKOSRemoval(name)
	if not Spy.db.profile.ShareKOSReasons then return end
	if not GetGuildInfo("player") then return end

	local escapedName = Spy:EscapeSyncString(name)
	local message = "KOSR|REM|"..escapedName.."|"..time()
	Spy:SendCommMessage(Spy.Signature, message, "GUILD")
end
	...
	if playerData then
		if not reason then
			...
			if reason == L["KOSReasonOther"] then
				if not other then
					StaticPopup_Show("Spy_SetKOSReasonOther", name, nil, name)
				else
					if other == "" then
						playerData.reason[L["KOSReasonOther"]] = nil
					else
						-- New format with attribution
						playerData.reason[L["KOSReasonOther"]] = {
							text = other,
						...
						-- Broadcast to guild
						Spy:BroadcastKOSReason(name, reason, other, timestamp)
					end
				...
				if playerData.reason[reason] then
					...
					-- New format with attribution
					playerData.reason[reason] = {
						addedBy = Spy.CharacterName,
						addedAt = timestamp,
						source = "local"
					}
					-- Broadcast to guild
					Spy:BroadcastKOSReason(name, reason, nil, timestamp)
				end
				Spy:RegenerateKOSCentralList(name)
			end
		end
...
-- Broadcast KOS reason to guild
function Spy:BroadcastKOSReason(name, reasonKey, reasonText, timestamp)
	if not Spy.db.profile.ShareKOSReasons then return end
	if not GetGuildInfo("player") then return end
	...
	local escapedText = Spy:EscapeSyncString(reasonText or "")
	local message = "KOSR|RSN|"..escapedName.."|"..escapedKey.."|"..escapedText.."|"..(timestamp or time())
	Spy:SendCommMessage(Spy.Signature, message, "GUILD")
end
			...
			if source ~= nil and source ~= Spy.CharacterName then
				...
				if playerData.reason then
					for reason, reasonData in pairs(playerData.reason) do
						...
						if reason == L["KOSReasonOther"] then
							-- Handle both old format (string) and new format (table with .text)
		...
		if characterName ~= Spy.CharacterName then
			...
			for player in pairs(characterKosData) do
				if not SpyDB.removeKOSData[Spy.RealmName][Spy.FactionName][player] then
					...
					-- Merge kosAddedBy (always merge, not time-dependent, keep earliest timestamp)
					if kosPlayerData.kosAddedBy then
						if not playerData.kosAddedBy then
							playerData.kosAddedBy = {}
						end
						for adderName, adderTimestamp in pairs(kosPlayerData.kosAddedBy) do
					...
					-- Merge guildStats (always merge, keep most recent per guild member)
					if kosPlayerData.guildStats then
						if not playerData.guildStats then
							playerData.guildStats = {}
						end
						for guildMember, stats in pairs(kosPlayerData.guildStats) do
							if not playerData.guildStats[guildMember] or
							   (playerData.guildStats[guildMember].lastUpdate or 0) < (stats.lastUpdate or 0) then
								playerData.guildStats[guildMember] = stats
							end
						end
					end
...
function Spy:ButtonClicked(self, button)
	...
	if name and name ~= "" then
		if button == "LeftButton" then
			...
			-- Normal left-click targeting is handled by the macrotext attribute set in SetBar
